...
export default {
    async fetch(request, env) {
        ...
        if (env.SITE_DB) {
            try {
                const tracks = (await catalogue(env, url.origin)).filter(track => track.key.startsWith(prefix));
                return jsonResponse({ tracks, count: tracks.length, prefix }, request, env, 200, { 'Cache-Control': 'no-store' });
            } catch { return jsonResponse({ error: 'Catalogue unavailable' }, request, env, 503); }
        }
...
async function streamAudio(url, request, env) {
    ...
    let key;
    try { key = decodePath(url.pathname.slice("/audio/".length)); } catch { return jsonResponse({ error: 'Invalid path' }, request, env, 400); }
    if (!isAudioKey(key) || !COLLECTIONS.some(prefix => key.startsWith(prefix)) || key.split('/').includes('..')) return jsonResponse({ error: 'Not found' }, request, env, 404);
