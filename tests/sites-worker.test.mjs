...
test('catalogue proxy only permits the three NCC collections and read requests', async () => {
    assert.equal((await worker.fetch(new Request('https://ncc.example/api/playlist?prefix=private/'), {})).status, 400);
    assert.equal((await worker.fetch(new Request('https://ncc.example/api/playlist?prefix=radio/', { method: 'POST' }), {})).status, 405);
...
test('catalogue proxy forwards to the fixed upstream and returns live data', async () => {
    ...
    try {
        globalThis.fetch = async target => {
            ...
            assert.equal(target.searchParams.get('prefix'), 'radio/');
            return Response.json({ tracks: [{ name: 'Current set' }] });
        };
        const response = await worker.fetch(new Request('https://ncc.example/api/playlist?prefix=radio/'), {});
