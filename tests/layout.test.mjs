...
test('user and timed comment fields share the compact action row', async () => {
    ...
    assert.match(css, /\.comment-user-compact \{[^}]*flex: 0 0 200px;[^}]*max-width: 200px;/);
    assert.match(css, /@media \(max-width: 760px\) \{[^]*\.comment-user-compact \{[^}]*flex-basis: 88px;[^}]*max-width: 92px;/);
