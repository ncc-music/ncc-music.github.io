...
test('user and timed comment fields share the compact action row', async () => {
    ...
    assert.match(css, /\.comment-user-compact \{[^}]*flex: 0 0 220px;[^}]*max-width: 220px;/);
    ...
    assert.match(css, /\.waveform-inline-form \.comment-composer \{[^}]*height: 38px;[^}]*min-height: 38px;/);
