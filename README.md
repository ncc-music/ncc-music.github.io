...
El sitio presenta una interfaz oscura y minimalista: live sets, filtros de colección, acceso directo a la radio y un reproductor fijo. La firma visual muestra “MIXED BY NICØLÁS CARDÚ”. La calavera mostaza HQ identifica el encabezado y el miniplayer, se integra con sus fondos oscuros y usa un tamaño adaptable ampliado; la lista reemplaza el play del set activo por un vúmetro animado en blanco y negro. La animación por piezas del reproductor ampliado se activa durante la reproducción y respeta la preferencia de movimiento reducido del dispositivo.
...
- **π (Administrar)** abre Tracklists después de iniciar sesión. Allí se editan nombre, enlace corto, fecha, tags, tracklist y estado publicado de los audios de R2. Cada cambio de enlace conserva los slugs anteriores como redirecciones permanentes. En la lista principal, el botón verde **Editar ficha** da acceso directo al editor completo, junto con los controles para subir o bajar cada set. El orden manual queda guardado; antes de editarlo, la colección se muestra de la más antigua a la más reciente. Un audio nuevo se incorpora al final al actualizar el catálogo; el audio pesado se sigue subiendo a R2.
- **Moderar comentarios** permite ocultar, volver a mostrar o eliminar mensajes desde el área privada. Los comentarios ocultos permanecen disponibles para el administrador, pero desaparecen de la ficha pública.
- **Ver estadísticas** resume reproducciones, sets completados, aperturas del reproductor ampliado y enlaces compartidos durante los últimos 30 días. Solo se guardan contadores agregados por día y set: no se almacenan cookies analíticas, IP ni perfiles personales.
...
- Cada set tiene una dirección breve y permanente, por ejemplo `/set/preview-002`. Si el nombre corto ya existe, el siguiente recibe `-2`, `-3`, etc. Cambiar el título no cambia la dirección y los enlaces largos anteriores redirigen automáticamente a la versión breve.
- Las fichas públicas incluyen metadatos sociales y datos estructurados `MusicPlaylist`. El sitemap dinámico enumera la portada y cada set publicado, incluso después de editar un slug.
...
- El miniplayer abre una vista Now Playing sin crear otro audio. Su calavera y la del encabezado tienen mayor presencia visual. La vista ampliada ocupa el 95% de la altura en escritorio y toda la pantalla en celulares. La animación se presenta en una franja horizontal compacta sobre el waveform, alterna reproducción y pausa y, en celulares, conserva esa acción táctil sin mostrar un segundo icono de Play sobre la imagen.
- La PWA muestra un aviso discreto cuando hay una versión nueva y solo la activa cuando la persona elige **Actualizar**, evitando reemplazar recursos durante una reproducción.
