(() => {
    ...
    const activateSilently = worker => worker?.postMessage({ type: 'SKIP_WAITING' });
    ...
    if ('serviceWorker' in navigator && location.protocol !== 'file:') {
        window.addEventListener('load', async () => {
            try {
                ...
                if (registration.waiting && navigator.serviceWorker.controller) activateSilently(registration.waiting);
                registration.addEventListener('updatefound', () => {
                    const worker = registration.installing;
                    worker?.addEventListener('statechange', () => {
                        if (worker.state === 'installed' && navigator.serviceWorker.controller) activateSilently(worker);
                    });
                });
