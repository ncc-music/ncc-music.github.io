...
(() => {
    ...
    function editSet(track) {
        ...
        $('edit-slug').value = editorTrack.slug || '';
    ...
    async function saveSet(event) {
        ...
        try {
            ...
            const slug = $('edit-slug').value.trim().toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
            await request('/admin/sets/' + editorTrack.id, { method: 'PUT', body: JSON.stringify({ title: $('edit-title').value, slug, date: $('edit-date').value, tags, sortOrder: editorTrack.sortOrder, tracklist: $('edit-tracklist').value.split('\n').map(line => line.trim()).filter(Boolean), animationPosterKey, animationVideoKey, published: $('edit-published').checked, version: editorTrack.version, peaks: editorTrack.peaks }) });
